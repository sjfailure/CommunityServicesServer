...
class TestHelpersInsertNewType(django.test.TestCase):
    """
    Test Suite for helpers.insert_new_service_type().
    """
    ...
    def test_creates_new_service_type_successfully(self):
        ...
        result = helpers.insert_new_service_type(
            type_name, self.category)
    ...
    def test_fails_on_invalid_category_type(self):
        """
        Ensure it raises TypeError if we pass a string instead of a
        model instance.
        """
        with self.assertRaises(TypeError):
            # Passing a string "Health" instead of the self.category
            # object
            helpers.insert_new_service_type("Dental", "Health")

    def test_idempotency_same_category(self):
        """
        Ensure it returns the existing type if name and category
        match.
        """
    ...
    #     """
    #     Unique constraint on ServiceType.type prevents multiple
    #     services of same type in separate categories.
    #     """
...
class TestHelpersRetrieveType(django.test.TestCase):
    """
    Test Suite for helpers.retrieve_type().
    """
