...
class TestHelpersInsertNewServiceEvent(TestCase):
    """
    Test Suite for helpers.insert_new_service_event().
    """
    ...
    def setUp(self):
        ...
        self.service_type = ServiceType.objects.create(
            type="ServiceType", category=self.service_category)
        self.second_service_category = ServiceCategory.objects.create(
            category="NewServiceCat")
        self.second_service_type = ServiceType.objects.create(
            type="ServiceType", category=self.second_service_category)
    ...
    def test_creates_new_service_event_happy_path(self):
        ...
        self.assertIn(self.service_category, [x.category for x in
                                              service.type.all()])
        ...
        self.assertEqual(
            service.start_time.isoformat(), "13:00:00")
        self.assertEqual(
            service.end_time.isoformat(), "14:00:00")
    ...
    def test_unique_service_event_happy_path(self):
        ...
        self.assertEqual(
            service.start_time.isoformat(), "09:00:00")
        self.assertEqual(
            service.end_time.isoformat(), "12:00:00")
        self.assertEqual(service.periodic, 0)
        ...
        self.assertEqual(
            service.start_time.isoformat(), "13:00:00")
        self.assertEqual(
            service.end_time.isoformat(), "16:00:00")
        self.assertEqual(service.periodic, 0)
        ...
        self.assertIsNotNone(service)
        self.assertEqual(Service.objects.count(), starting_db_count)
        # no new object created

    def test_admin_return_flag_works(self):
        """
        Returns a tuple (Service obj, Boolean) if "report_status" flag
        activated. Flag should be True if Service obj created
        successfully, False if attempt failed because it was a
        duplicate.
        """
        starting_count = Service.objects.count()
        service, status = helpers.insert_new_service_event(
            category=self.service_category,
             service_type=self.service_type,
             provider=self.provider,
             audience=self.audience,
        ...
        self.assertEqual(service.periodic, 0)
        self.assertEqual(service.note, 'notes')
        # all data should be correct and present, but the status
        # will have failed and object not added to the table
    ...
    def test_create_with_multiple_categories_and_audiences(self):
        """
        Verify a single service can be tagged with multiple M2M
        objects during creation.
        """
        audi2 = Audience.objects.create(audience="Veterans")

        service = helpers.insert_new_service_event(
            category=None,  # OBSOLETE, CATEGORY REFERENCED VIA
        ...
        # Verify counts
        self.assertEqual(
            len({x.category for x in service.type.all()}), 2)
        self.assertEqual(service.audience.count(), 2)

        # Verify specific members
        self.assertIn(
            self.second_service_category,
            [x.category for x in service.type.all()]
        )
    ...
    def test_update_existing_service_adds_new_category(self):
        """
        Verify that calling the helper for an existing time-slot adds
        new M2M relationships rather than creating a duplicate Service
        record.
        """
        ...
        service, created = helpers.insert_new_service_event(
            category=None, # OBSOLETE, CATEGORY REFERENCED FROM
            # SERVICE_TYPE.CATEGORY
        ...
        # Should have found the existing record
        self.assertEqual(Service.objects.count(), starting_count)
        # No new DB row

        # 3. Verify the existing record now has BOTH categories
        self.assertEqual(
            len({x.category for x in service.type.all()}),
            2
        )
        self.assertIn(
    ...
    def test_create_service_with_multiple_days_list(self):
        """
        Verify passing a list of days tags a single service with
        all of them.
        """
    ...
    def test_adding_new_day_to_existing_service_consolidates(self):
        """
        Verify that calling the helper with a new day adds it to the
    ...
    def test_day_idempotency(self):
        """
        Verify that adding the same day twice doesn't create
        duplicate M2M links.
        """
        service = helpers.insert_new_service_event(
            category=self.service_category,
